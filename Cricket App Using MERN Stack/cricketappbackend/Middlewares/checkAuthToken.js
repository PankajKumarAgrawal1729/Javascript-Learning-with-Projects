...
function checkAuth(req, res, next) {
  //get the expire of tokens
  const authToken = req.cookies.authToken; //50 min
  const refreshToken = req.cookies.refreshToken; //120 min
    ...
    if (err) {
      jwt.verify(
        ...
        (refreshErr, refreshDecoded) => {
          if (refreshErr) {
            //Both tokens are invalid, send an error message and promt for login
            return res.status(401).json({
